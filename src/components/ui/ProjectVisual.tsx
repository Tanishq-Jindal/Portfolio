...
export const ProjectVisual: React.FC<ProjectVisualProps> = ({ projectId }) => {
  ...
  if (projectId === "samaadhan") {
    const pipelineSteps = [
      {
        name: "Citizen Report",
        desc: "GPS coordinates & photo capture",
        icon: <Camera className="w-3.5 h-3.5 text-emerald-400" />,
      },
      {
        name: "Interactive Map",
        desc: "Geospatial incident pinning",
        icon: <MapPin className="w-3.5 h-3.5 text-indigo-400" />,
      },
      {
        name: "Auto Routing",
        desc: "Category-based department triage",
        icon: <Layers className="w-3.5 h-3.5 text-cyan-400" />,
      },
      {
        name: "Admin Dispatch",
        desc: "Triage resolution dashboard",
        icon: <CheckCircle2 className="w-3.5 h-3.5 text-amber-400" />,
      },
      {
        name: "Municipal Action",
        desc: "Field resolution & verification",
        icon: <Workflow className="w-3.5 h-3.5 text-indigo-400" />,
      },
      {
        name: "Citizen Sync",
        desc: "Live ticket tracking & audit",
        icon: <Eye className="w-3.5 h-3.5 text-emerald-400" />,
      },
    ];
      ...
      <div className="rounded-xl overflow-hidden border border-slate-800 bg-[#090d16] shadow-xl p-5 space-y-4">
        ...
        <div className="pt-2 border-t border-slate-800/70 flex flex-wrap items-center justify-between gap-2 text-xs font-mono text-slate-400">
          ...
          <div className="flex flex-wrap gap-2 text-[11px]">
            ...
            <span className="text-slate-600">·</span>
            <span className="text-slate-300">Geolocation API</span>
          </div>
        </div>
      </div>
    );

      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-stretch">
          {educationList.map((item, index) => (
            <div
              key={index}
              className="p-5 sm:p-6 rounded-2xl bg-slate-900/40 border border-slate-800/80 hover:border-slate-700/80 transition-all backdrop-blur-sm flex flex-col justify-between h-full space-y-4"
            >
              ...
              {item.courseworkHighlights && item.courseworkHighlights.length > 0 && (
                <div className="pt-2.5 border-t border-slate-800/60 space-y-2 mt-auto">
